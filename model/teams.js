  ...
  for (let team in game.Teams) {
    for (let player in team.Players) {
      if (team.TeamId === my_team_id)
        this.my_team.append(new PLayer(player["PlayerColor"], player["HeroType"]))
      else
        this.enemy_team.append(new PLayer(player["PlayerColor"], player["HeroType"]))
    }
  }
}
...
get_team_colors_by_color = (player_color) => {
  ...
  __get_team_id = (self, player_color) => {
