...
+ Не вносить изменения в файлы проекта кроме `bot.js` и `index.js` (только в части задания значений `USERID` и `BOTID`).
+ Не менять имя метода `Bot`.
+ Написание кода происходит в методе `Bot` файла `bot.js`.
...
+ для сборки и запуска приложения необходимо локально установить node.js. Инструкция по установке на разных ОС: https://nodejs.org (на MacOS используй brew install node)
+ должен быть установлен менеджер пакетов yarn. Инструкция по установке на разных ОС: https://classic.yarnpkg.com/ (на MacOS используй brew install yarn)
+ должен быть установлен плагин Node.js
+ для установки пакетов используем файл package.json
+ для запуска приложения рекомендуется использовать конфигурацию Node.js, JavaScript file - index.js
